...
EXPORT_URL = os.getenv(
    "EXPORT_URL",
    "http://localhost:8000/v1/samples/export?format=ndjson",
)
...
def fetch_latest_ndjson(export_url: str, data_path: Path) -> None:
    print(f"Fetching latest samples from API: {export_url}")
    data_path.parent.mkdir(parents=True, exist_ok=True)
    with urllib.request.urlopen(export_url) as resp:
        data_path.write_bytes(resp.read())
    print(f"Saved fresh dataset snapshot to: {data_path}")
...
def build_xy(rows):
    ...
    if not X:
        raise ValueError("No valid samples found after parsing dataset.")
...
def main():
    fetch_latest_ndjson(EXPORT_URL, DATA_PATH)
